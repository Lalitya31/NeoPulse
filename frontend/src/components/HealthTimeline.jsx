...
function DayCard({ day, isToday, isSelected, onClick, zoom }) {
  ...
  return (
    <div
      ...
      onMouseLeave={e => { if (!isSelected) e.currentTarget.style.background = isToday ? "rgba(96,165,250,0.06)" : "rgba(255,255,255,0.025)"; }}
    >
      {/* Date header */}
      <div style={{ marginBottom: 6 }}>
        <div style={{ fontSize: 7, color: "rgba(255,255,255,0.25)", letterSpacing: 2 }}>
          {dayName.toUpperCase()} {monthAbbr.toUpperCase()}
        </div>
        <div style={{
          fontSize: zoom === "week" ? 18 : 22,
          fontWeight: 700, color: isToday ? "#60a5fa" : "#fff",
          lineHeight: 1,
        }}>
          {dayNum}
      ...
      {zoom !== "week" && (
        <>
          ...
          <div style={{ marginBottom: 5 }}>
            <div style={{ fontSize: 7, color: "rgba(255,255,255,0.2)", letterSpacing: 1, marginBottom: 2 }}>
              MOOD
            </div>
...
export default function HealthTimeline({ token }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`${API}/dashboard/timeline?days=${range}`, {
  ...
  return (
    <div style={{
      fontFamily: "'DM Mono', monospace",
      background: "linear-gradient(180deg, #060612 0%, #09091a 100%)",
      minHeight: "100vh",
      padding: "28px 24px",
      color: "#fff",
    }}>
      <style>{`
        ...
        .zoom-btn:hover { background: rgba(255,255,255,0.1) !important; }
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 8, color: "rgba(255,255,255,0.25)", letterSpacing: 4, marginTop: 2 }}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        {[
          ...
          { label: "JOURNAL DAYS",val: journalDays,  color: "#60a5fa" },
          { label: "DAYS TRACKED",val: days.length,  color: "rgba(255,255,255,0.4)" },
        ].map(({ label, val, color }) => (
          <div key={label} style={{
            background: "rgba(255,255,255,0.03)",
            border: "1px solid rgba(255,255,255,0.06)",
            ...
            <div style={{ fontSize: 8, color: "rgba(255,255,255,0.25)", letterSpacing: 2, marginBottom: 3 }}>
              {label}
            </div>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {["day", "week", "month"].map(z => (
            <button key={z} className="zoom-btn" onClick={() => setZoom(z)} style={{
              padding: "5px 12px",
              background: zoom === z ? "rgba(255,255,255,0.1)" : "transparent",
              border: `1px solid ${zoom === z ? "rgba(255,255,255,0.2)" : "rgba(255,255,255,0.06)"}`,
              borderRadius: 5, color: zoom === z ? "#fff" : "rgba(255,255,255,0.3)",
      ...
      >
        {loading ? (
          ...
          <div key={day.date} data-today={day.date === today ? "true" : undefined}>
            <DayCard
              day={day}
      ...
      {selectedDay && (
        <div style={{
          marginTop: 20,
          background: "rgba(255,255,255,0.03)",
          border: `1px solid ${sentimentColor(selectedDay.sentiment)}33`,
          borderRadius: 12,
          padding: "16px 20px",
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
            <div>
              ...
              <div style={{ fontSize: 8, color: "rgba(255,255,255,0.3)", letterSpacing: 2, marginTop: 2 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
            {[
              ...
              { label: "Journal",          val: selectedDay.has_journal ? "✓ Written" : "○ None", color: selectedDay.has_journal ? "#60a5fa" : "rgba(255,255,255,0.3)" },
              { label: "Emotion Session",  val: selectedDay.has_session ? "✓ Active" : "○ None", color: selectedDay.has_session ? "#a78bfa" : "rgba(255,255,255,0.3)" },
            ].map(({ label, val, color }) => (
              <div key={label}>
